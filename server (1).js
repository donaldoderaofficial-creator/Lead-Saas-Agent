...
const { pendingLeads, completedReports, leads, users, subscription } = require('./store');
...
app.post('/payments/capture-order/:orderId', async (req, res) => {
  try {
    ...
    if (capture.result.status === 'COMPLETED') {
      ...
      if (customId.startsWith('annual:')) {
        activateAnnualPlan(customId.split(':')[1]);
      } else {
...
function activateAnnualPlan(plan) {
...
// subscribed to PAYMENT.CAPTURE.COMPLETED, BILLING.SUBSCRIPTION.ACTIVATED, and
// BILLING.SUBSCRIPTION.CANCELLED. Set PAYPAL_WEBHOOK_ID to the ID it gives you.
// Every event is verified against PayPal before being trusted.
app.post('/payments/paypal/webhook', async (req, res) => {
  ...
  if (event.event_type === 'BILLING.SUBSCRIPTION.ACTIVATED') {
    ...
    const planName =
      planId === process.env.PAYPAL_PLAN_GROWTH_MONTHLY ? 'growth' :
  ...
  if (event.event_type === 'BILLING.SUBSCRIPTION.CANCELLED' || event.event_type === 'BILLING.SUBSCRIPTION.SUSPENDED') {
    ...
    if (current.paypalSubscriptionId === event.resource?.id) {
      subscription.set({ ...current, status: 'cancelled' });
    }
  }
...
// ---- Billing: plans, pricing, and subscription status ----
const PLAN_PRICING_USD = { starter: 79, growth: 249 };
const ANNUAL_DISCOUNT_MONTHS = 2; // "pay for 10 months, get 12" style discount

// Public — the frontend needs the client ID and plan IDs to render PayPal's
// subscribe buttons. None of these values are secret (client ID is meant to
// be public; plan IDs aren't sensitive on their own).
app.get('/api/config', (req, res) => {
  res.json({
    paypalClientId: process.env.PAYPAL_CLIENT_ID,
    plans: {
      starter: { priceMonthly: PLAN_PRICING_USD.starter, priceAnnual: PLAN_PRICING_USD.starter * (12 - ANNUAL_DISCOUNT_MONTHS), paypalPlanId: process.env.PAYPAL_PLAN_STARTER_MONTHLY },
      growth: { priceMonthly: PLAN_PRICING_USD.growth, priceAnnual: PLAN_PRICING_USD.growth * (12 - ANNUAL_DISCOUNT_MONTHS), paypalPlanId: process.env.PAYPAL_PLAN_GROWTH_MONTHLY },
    },
  });
});
...
// Annual one-time payment — reuses the same order/capture pattern as lead
// payments, just tagged with a custom_id the capture handler recognizes.
app.post('/api/billing/annual', requireAuth, async (req, res) => {
  const { plan } = req.body || {};
  if (!PLAN_PRICING_USD[plan]) {
    return res.status(400).json({ error: "plan must be 'starter' or 'growth'" });
  }
  const price = (PLAN_PRICING_USD[plan] * (12 - ANNUAL_DISCOUNT_MONTHS)).toFixed(2);
  ...
  try {
    ...
    createReq.requestBody({
      ...
      purchase_units: [
        {
          description: `${plan} plan — annual`,
          custom_id: `annual:${plan}`,
          amount: { currency_code: 'USD', value: price },
        },
      ],
      application_context: {
        return_url: `${req.protocol}://${req.get('host')}/billing.html?paypal_return=1`,
        cancel_url: `${req.protocol}://${req.get('host')}/billing.html?paypal_cancel=1`,
      },
    });
    const order = await client().execute(createReq);
    const approveUrl = order.result.links?.find((l) => l.rel === 'approve')?.href;
...
// Gate for Growth-only features. Plan-gating is account-wide, not per dashboard user.
function requireGrowthPlan(req, res, next) {
  const sub = subscription.get();
  if (sub.plan !== 'growth' || sub.status !== 'active') {
    return res.status(402).json({ error: 'This feature requires an active Growth plan subscription' });
  }
  next();
}
...
app.post('/api/prospecting/companies', requireAuth, requireGrowthPlan, async (req, res) => {
  try {
...
app.post('/api/prospecting/person', requireAuth, requireGrowthPlan, async (req, res) => {
...
app.post('/api/prospecting/company-prospects', requireAuth, requireGrowthPlan, async (req, res) => {
